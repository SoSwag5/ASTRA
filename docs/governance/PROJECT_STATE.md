...
- v1.1 implementation status: **issue #37 (Discovery Query Planner) COMPLETE**
  ...
  **COMPLETE and merged** to `master` via
  ...
  -- which is also fixed: the two queues now share one round-robin budget and
  `considered + revisited` never exceeds the requested limit. This remains **implementation and self-verification**,
...
`feature/46-application-reconciliation-state-model`. All five independently
reproduced defects across two review rounds are fixed with regression
coverage; local regression,
