...
|---|---|---|---|
| Product scope and local-first trust model | Implemented; governance accepted | Frozen source, README, architecture inventory, threat model, local-access and privacy docs | ADR-0001 accepted 2026-09-13; reassess on its documented triggers. |
...
| Access-key/session exchange | Implemented; governance accepted | Frozen implementation, session/security tests, local-access documentation | ADR-0003 accepted 2026-09-13; reassess on identity or trust-boundary changes. |
| Build once / verify same artifact | Executed; governance accepted | Candidate run 34722561421, final review run 34732173819, released artifact digest | ADR-0004 accepted 2026-09-13. Reverify each future candidate. |
| CycloneDX 1.7 SBOM | Executed; governance accepted | 246 components, schema validation and SBOM attestation; released digest recorded | ADR-0005 accepted 2026-09-13. Regenerate and validate for future dependency changes. |
| Hosted provenance | Executed and verified; governance accepted | Provenance and SBOM attestations verified against source/artifact; release/gate records | ADR-0006 accepted 2026-09-13. Preserve the evidence-specific SLSA wording. |
...
| ADR history | Complete for Governance v1 | ADR-0001 through ADR-0006 are Accepted; ADR-0002 retains the existing R-15 decision | Reassess only when an ADR trigger occurs. |
...
Governance backfill is complete for the six initial ADRs. Issues #24-#27 remain
as post-release follow-up. Those changes belong to later repository state and do
not modify the v1.0.0 tag or artifacts.
